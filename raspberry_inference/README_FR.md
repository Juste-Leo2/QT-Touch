...
**Note :** Cette configuration est spécifiquement prévue pour une portion de la veste, plus précisément le **torse droit du robot**.
...
Pour le branchement des capteurs de la veste (capteur piézo-résistif), une résistance de **47 kΩ** est requise pour assurer la stabilité des mesures. 
**Attention :** pour le capteur piézo-résistif, il faut impérativement prendre les pinces avec un "P" comme initiale.
...
### 6. Inférence en lien avec le projet QTRobot-Interaction
Si vous utilisez ce code dans le cadre du projet [QTRobot-Interaction](https://github.com/Juste-Leo2/QTRobot-Interaction), le processus d'installation suit les mêmes étapes (1 à 5).

La seule différence réside dans la configuration de la classe `RaspberryManager` exécutée dans le `main.py` de l'autre dépôt (`QTRobot-Interaction`). Ici il faudra adapter au Raspberry Pi en question avec le bon chemin :
